    ...
    <form id="formContrato" method="POST"
        ...
        <div class="form-grid">
            <div class="form-group">
                ...
                <select name="propiedad_id" id="propiedad_id">
                    ...
                    <?php foreach ($propiedades as $p): ?>
                        <option value="<?= $p['id'] ?>" <?= $p['id'] == ($contrato['propiedad_id'] ?? '') ? 'selected' : '' ?>>
                            <?= htmlspecialchars($p['direccion']) ?> (<?= htmlspecialchars($p['estado']) ?>)
                        </option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="form-group">
                ...
                <select name="inquilino_id" id="inquilino_id">
                    ...
                    <?php foreach ($inquilinos as $i): ?>
                        <option value="<?= $i['id'] ?>" <?= $i['id'] == ($contrato['inquilino_id'] ?? '') ? 'selected' : '' ?>>
            ...
            <div class="form-group">
                ...
                <input type="date" name="fecha_inicio" id="fecha_inicio" value="<?= htmlspecialchars($contrato['fecha_inicio'] ?? '') ?>">
                <span class="field-error" id="err_fecha_inicio"></span>
            </div>
            <div class="form-group">
                <label>Fecha de fin <span class="required">*</span></label>
                <input type="date" name="fecha_fin" id="fecha_fin" value="<?= htmlspecialchars($contrato['fecha_fin'] ?? '') ?>">
                <span class="field-error" id="err_fecha_fin"></span>
            </div>
            <div class="form-group">
                <label>Monto mensual ($) <span class="required">*</span></label>
                <input type="number" name="monto_mensual" id="monto_mensual" step="0.01" min="0.01" value="<?= htmlspecialchars($contrato['monto_mensual'] ?? '') ?>">
            ...
            <div class="form-group">
                ...
                <select name="estado">
                    <?php foreach (['Activo','Finalizado','Cancelado'] as $est): ?>
                        <option value="<?= $est ?>" <?= $est === ($contrato['estado'] ?? '') ? 'selected' : '' ?>><?= $est ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="form-group full">
                <label>Observaciones</label>
                <textarea name="observaciones"><?= htmlspecialchars($contrato['observaciones'] ?? '') ?></textarea>
            </div>
        </div>
...
document.getElementById('formContrato').addEventListener('submit', function(e) {
    ...
    if (!propiedad.value) { propiedad.classList.add('input-error'); document.getElementById('err_propiedad').textContent = 'Selecciona una propiedad'; valido = false; }
    if (!inquilino.value) { inquilino.classList.add('input-error'); document.getElementById('err_inquilino').textContent = 'Selecciona un inquilino'; valido = false; }
    if (!fechaInicio.value) { fechaInicio.classList.add('input-error'); document.getElementById('err_fecha_inicio').textContent = 'La fecha de inicio es obligatoria'; valido = false; }
    if (!fechaFin.value) { fechaFin.classList.add('input-error'); document.getElementById('err_fecha_fin').textContent = 'La fecha de fin es obligatoria'; valido = false; }
    if (fechaInicio.value && fechaFin.value && fechaFin.value <= fechaInicio.value) { fechaFin.classList.add('input-error'); document.getElementById('err_fecha_fin').textContent = 'La fecha de fin debe ser mayor a la de inicio'; valido = false; }
    if (!monto.value || parseFloat(monto.value) <= 0) { monto.classList.add('input-error'); document.getElementById('err_monto').textContent = 'El monto debe ser mayor a 0'; valido = false; }
