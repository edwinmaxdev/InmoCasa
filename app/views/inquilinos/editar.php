...
<style>
    ...
    .alert-error { background: #fef2f2; border: 1px solid #fecaca; border-radius: 8px; padding: 0.85rem 1rem; margin-bottom: 1.5rem; font-size: 0.875rem; color: #dc2626; display: flex; gap: 0.5rem; align-items: flex-start; }
...
<div class="page-header">
    ...
    <div class="page-title">
        <i class="fa-solid fa-pen"></i> Editar inquilino #<?= $inquilino['id'] ?? '' ?>
    </div>
</div>
    ...
    <form id="formInquilino" method="POST"
        ...
        <div class="form-grid">
            <div class="form-group full">
                ...
                <input type="text" name="nombre" value="<?= htmlspecialchars($inquilino['nombre'] ?? '') ?>">
                <span class="field-error" id="err_nombre"></span>
            </div>
            <div class="form-group">
                <label>Cédula <span class="required">*</span></label>
                <input type="text" name="cedula" value="<?= htmlspecialchars($inquilino['cedula'] ?? '') ?>">
                <span class="field-error" id="err_cedula"></span>
            </div>
            <div class="form-group">
                <label>Teléfono</label>
                <input type="text" name="telefono" value="<?= htmlspecialchars($inquilino['telefono'] ?? '') ?>">
            </div>
            <div class="form-group">
                <label>Email <span class="required">*</span></label>
                <input type="email" name="email" value="<?= htmlspecialchars($inquilino['email'] ?? '') ?>">
                <span class="field-error" id="err_email"></span>
            </div>
            <div class="form-group">
                <label>Dirección</label>
                <input type="text" name="direccion" value="<?= htmlspecialchars($inquilino['direccion'] ?? '') ?>">
            </div>
            <div class="form-group full">
                <label>Referencia</label>
                <textarea name="referencia"><?= htmlspecialchars($inquilino['referencia'] ?? '') ?></textarea>
            </div>
        </div>
