...
<div class="page-header">
    ...
    <div class="page-title"><i class="fa-solid fa-pen"></i> Editar propiedad #<?= $propiedad['id'] ?? '' ?></div>
</div>
...
<div class="form-card">
    <form id="formPropiedad" method="POST" action="<?= BASE_URL ?>?action=propiedad_actualizar&id=<?= $propiedad['id'] ?? '' ?>" novalidate>
        <div class="form-grid">
            <div class="form-group full">
                <label>Dirección <span class="required">*</span></label>
                <input type="text" name="direccion" value="<?= htmlspecialchars($propiedad['direccion'] ?? '') ?>">
                <span class="field-error" id="err_direccion"></span>
            </div>
            <div class="form-group">
                <label>Precio <span class="required">*</span></label>
                <input type="number" step="0.01" name="precio" value="<?= htmlspecialchars($propiedad['precio'] ?? '') ?>">
                <span class="field-error" id="err_precio"></span>
            </div>
            <div class="form-group">
                <label>Metros Cuadrados (m²) <span class="required">*</span></label>
                <input type="number" step="0.01" name="metros2" value="<?= htmlspecialchars($propiedad['metros2'] ?? '') ?>">
            ...
            <div class="form-group">
                ...
                <select name="tipo_id" id="tipo_id">
                    ...
                    <?php foreach ($tipos as $t): ?>
                        <option value="<?= $t['id'] ?>" <?= ($propiedad['tipo_id'] == $t['id']) ? 'selected' : '' ?>><?= htmlspecialchars($t['nombre']) ?></option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="form-group">
                ...
                <select name="propietario_id" id="propietario_id">
                    ...
                    <?php foreach ($propietarios as $p): ?>
                        <option value="<?= $p['id'] ?>" <?= ($propiedad['propietario_id'] == $p['id']) ? 'selected' : '' ?>><?= htmlspecialchars($p['nombre']) ?></option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="form-group">
                ...
                <select name="estado">
                    ...
                    <option value="En venta" <?= ($propiedad['estado'] === 'En venta') ? 'selected' : '' ?>>En venta</option>
                    <option value="Vendida" <?= ($propiedad['estado'] === 'Vendida') ? 'selected' : '' ?>>Vendida</option>
                </select>
            </div>
            <div class="form-group full">
                <label>Descripción</label>
                <textarea name="descripcion" rows="4"><?= htmlspecialchars($propiedad['descripcion'] ?? '') ?></textarea>
            </div>
        </div>
